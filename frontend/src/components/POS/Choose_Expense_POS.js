  ...
  const fetch_data = () => {
    onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        axios
          .post(API_LINK + "get_warehouse_By_company", {
  ...
  return (
    <div>
      ...
      {warehouse_index ? (
        ...
        <Create_Expense warehouse_name={warehouse_data[0].title} />
      ) : (
        ""
      )}
    </div>
...
export default Choose_Expense_POS;
